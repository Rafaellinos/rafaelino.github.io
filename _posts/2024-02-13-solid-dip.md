...
O que exatamente essa frase quer dizer? Basicamente, que não deveríamos depender de classes concretas, mas sim, de abstrações.

Se você está familiarizado com POO, provavelmente já entendeu de cara o significado, caso não, iremos discuti-los em Java, especificamente.
...
Em programação orientada a objetos (POO), abstrações são classes que não podem ser instânciadas diretamente, e, em geral, são utilizadas para definir um comportamento em comum.
Abstrações serão usados em heranças e terão diferentes comportamentos dependendo da implementação (Polimorfismo). Como podemos ver, vários pilares de POO estão envolvidos com DIP.
...
A implementação de DIP é bem simples, por norma, não injete dependências diretas na sua classe, mencionando classes, métodos, funções, etc concretas. 
...
Acima vemos um caso claro de quebra de DIP, pois dependemos de duas classes concretas. Estamos quebrando OCP também, pois, caso uma nova forma de pagamento seja necessária, precisaríamos editar a classe `ProcessadorPagamento` diretamente.
...
public class FormaPagamento { // Abstração
...
public interface FormaPagamentoFactory { // Fabrica abstrata
...
public class ProcessadorPagamento {
  ...
  public ProcessadorPagamento(FormaPagamentoFactory formaPagamentoFactory) { // dependemos de uma abstração
  ...
  public void processarPagamento(Dados dados) {
    ...
    formaPagamento.processarPagamento(dados); // formaPagamento é abstrato, não precisamos detalhes de sua implementação
  }
}
```
...
Utilizamos uma Abstract Factory para construir nossas classes concretas, além de utilizarmos o Strategy pattern.
...
- OCP = Caso precisássemos criar uma nova foram de pagamento, teríamos que modificar `FormaPagamentoFactoryImpl`, quebrando OCP.

Seria possível resolver essas duas quebras com Registry pattern, utilizando um `HashMap` com as keys (Enums) e values (Classes concretas) no Abstract Factory e instanciar cada uma dinamicamente, ou utilizar alguma outra técnica como Locator Pattern, mas todas elas possuem seus trade-offs e cabe ao desenvolvedor avaliar os requisitor não funcionais, como desempenho, flexibilidade necessária, etc.
Lembra-se que SOLID serve de guia e não precisa ~~eu diria que nem deve, se não você fica maluco~~ ser seguido a risca em todos os níveis.
...
- DI = É uma implementação do princípio IoC, sendo considerado um padrão que o próprio String utiliza (A exemplo do `@Autowired`).
- DIP = Principio que nos guia a evitar acoplamento referenciando classes e métodos concretos.

## Conclusão

DIP é um princípio poderoso e combinado com Design patterns, e podem facilitar a vida util (manutenabilidade) do seu projeto, melhorando até mesmo o desempenho do seu time na construção de features a longo prazo, visto que o mesmo provê uma base de código extensível e flexível.
